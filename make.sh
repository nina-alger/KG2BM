...
${NEO_USER} cypher-shell \
    --database oncodash \
    --username neo4j \
    --password $(cat neo4j.pass) "MATCH (g:Gene) WHERE g.gene_symbol IN ['PIK3CA', 'MAP2K1', 'AKT1', 'MAP3K7', 'MAPK14', 'GSK3B', 'CTNNB1'] RETURN g.gene_symbol;" | tail -n +2 | tr -d '"' > protein_list.txt
